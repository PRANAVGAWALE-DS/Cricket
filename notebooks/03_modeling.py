...
print(
    f"Training on {len(win_prob_feats)} over-snapshots from {win_prob_feats['match_id'].nunique()} matches"
)
...
try:
    ...
    fig.add_annotation(
        x=curve["over"].max(),
        y=curve["win_probability"].iloc[-1],
        text=outcome_text,
...
X_wp = win_prob_feats.drop(
    columns=[c for c in drop_cols if c in win_prob_feats.columns]
)
    ...
    [
        {
            ...
            "Type": "Classifier",
            "Algorithm": "XGBoost",
            "Key Metric": f"AUC = {metrics_winner['roc_auc']}",
            "Accuracy": metrics_winner["accuracy"],
        },
        {
            "Model": "Score Predictor",
            "Type": "Regressor",
            "Algorithm": "LightGBM",
            "Key Metric": f"MAE = {metrics_score['mae']} runs",
            "Accuracy": metrics_score["r2"],
        },
        {
            "Model": "Win Probability",
            "Type": "Classifier",
            "Algorithm": "LightGBM",
            "Key Metric": f"AUC = {metrics_wp['roc_auc']}",
            "Accuracy": metrics_wp["accuracy"],
        },
        {
            "Model": "POTM Predictor",
            "Type": "Classifier",
            "Algorithm": "XGBoost",
            "Key Metric": f"AUC = {metrics_potm['roc_auc']}",
            "Accuracy": metrics_potm["accuracy"],
        },
    ]
)
...
# Feature formulas must match build_win_probability_features exactly:
#   current_rr  = runs_scored / over          (not runs / (over/6) !)
#   required_rr = runs_required / (balls_remaining / 6)
# Q6 note: the predicted-vs-actual scatter in section 2 re-splits with
# random_state=42, which matches train_score_predictor's internal split.
# If that seed ever changes, re-split here must change to match.
over_number = 15
runs_scored = 110
balls_remaining = 30
runs_required = 60
    ...
    [
        {
            ...
            "wickets_fallen": 3,
            "current_rr": round(runs_scored / over_number, 2),  # 7.33
            "required_rr": round(runs_required / (balls_remaining / 6), 2),  # 12.0
