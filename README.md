...
| **GRU Score Predictor** | PyTorch GRU | R² | **0.33** | Sequence model, 756 matches |
...
**On GRU vs LightGBM:** The GRU's architectural advantage is over-by-over inference — it accepts live ball data as it arrives, while LightGBM requires a complete over-10 snapshot. On 756 matches the metrics are comparable; adding Cricsheet data (~5,000 matches) is expected to push GRU MAE below LightGBM's 18 runs.
...
make train         # run notebooks 03 → 04 → 06 → 05
...
| 📈 **Win Probability** | Select any historical match → animated over-by-over win probability curve |
...
- [ ] Fantasy points optimizer (knapsack team selector)
